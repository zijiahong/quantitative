...
def on_tick(context,tick):
    ...
    k,d,j = KDJ(pd.Series(high+[max]),pd.Series(low+[min]),pd.Series(close+[tick.quotes[0]['ask_p']]))
    ...
    if position != None and position.vwap - tick.price > 0.03:
    ...
    # k 和 d 收敛,d 大于 k 
    if  k.iloc[-1] > d.iloc[-1] and j.iloc[-1] > k.iloc[-1]:
        tick_kd +=1
    else:
        tick_kd = 0
    
   
    if tick_kd >=5 and j.iloc[-1] < 20:
        # 买进
...
def on_bar(context,bars):
    ...
    if len(high) > 30 :
        ...
        if d.iloc[-1]>k.iloc[-1] and abs(d.iloc[-1]-k.iloc[-1])>5:
            if bar_dk == 0 and d.iloc[-1]-k.iloc[-1]>15:
                bar_dk +=1
            elif bar_dk == 1 and d.iloc[-1]-k.iloc[-1]>10:
                bar_dk +=1
            else:
                bar_dk +=1
        else:
